...
from .services import get_subscribers, create_subscriber, delete_subscriber, search_subscribers
...
async def get_my_subscribers_api_view(
    user: User = Depends(get_current_user),
    session=Depends(get_session),
):
    return await get_subscribers(session, user.id)
...
@router.get("/search", response_model=list[SubscriberSchema])
async def search_subscribers_api_view(
    search: str = Query(..., min_length=3, max_length=100),
    _=Depends(get_current_user),
    session=Depends(get_session),
):
    return await search_subscribers(session, search)
